...
PRIVATE_KEY=/var/lib/rundeck/scripts/stage/stage_stop_start/ubuntu.pem
...
RIAK_USERNAME=nessus
DATE=`date +%F`
TIME=`date +%H-%M-%S`
EMAIL_FROM=ubuntu-alerts@jar.com
EMAIL_RCPTS=ubuntu-alerts@jar.com
...
playelb="dr-cloud-play-cluster-lb"
#play_tag="Stage-Play"

playapilist=/tmp/stage/playapi_instance.txt
playapielb="dr-cloud-api-cluster-lb"
...
riakelb="dr-cloud-riak-cluster-lb"
#riak_tag="Stage-Riak"

xraylist=/tmp/stage/xray_instance.txt
xrayelb="dr-cloud-xray-cluster-lb"
#xray_tag="Stage-Xray"
...
if  [ "InService" != "$status" ] ; then
      echo "detaching the instance from elb $playapielb" | tee $MAILLOG
...
IP=`aws ec2 describe-instances --instance-ids  "$i" --query 'Reservations[*].Instances[*].[PublicIpAddress]' --output text --region $REGION`
ssh -q -i $RIAK_KEY -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null $RIAK_USERNAME@$IP "sudo sh -c 'echo deadline > /sys/block/xvda/queue/scheduler'"
ssh -q -i $RIAK_KEY -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null $RIAK_USERNAME@$IP "sudo sh -c 'echo 1024 > /sys/block/xvda/queue/nr_requests'"
...
riak_handoff_check() {
instance=`cat $riaklist | awk '{print $1}' |  head -1` 
IP=`aws ec2 describe-instances --instance-ids  "$instance" --query 'Reservations[*].Instances[*].[PublicIpAddress]' --output text --region $REGION`
echo "checking the stage riak handoff status" | tee $MAILLOG
status=`ssh -q -i $RIAK_KEY -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null $RIAK_USERNAME@$IP 'sudo riak-admin transfers' | grep handoff | awk '{print $4}' |head -1`
while [ "handoff" = "$status" ] ; do
      echo "wait for handoff transfer to complete and  sleep 60sec" | tee $MAILLOG
      sleep 60
      status=`ssh -q -i $RIAK_KEY -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null $RIAK_USERNAME@$IP 'sudo riak-admin transfers' | grep handoff | awk '{print $4}' |head -1`
done
echo "Riak handoff transfer is completed" | tee $MAILLOG
}
...
riak_discover
riak_stop
elif [ "$ACTION" = "stage_bring_up" ];then
