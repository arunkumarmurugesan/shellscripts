...
echo "---------START--------------"
ssh -q -i $PRIVATE_KEY -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null $USERNAME@$IP "docker exec $DOCKER_NAME df -H | grep -vE '^Filesystem|tmpfs' | awk '{ print $5 " " $1 }'" | tee -a $FS_CHECK
echo "---------END--------------"
while read line ; do  
